...
class TestSum(unittest.TestCase):
    ...
    def test_check_reg_ok(self):
        user = w3.eth.accounts[9]
        login = regTeacher_ok[0][0]
        ...
        tx_hash = cont_teacher_0.functions.registrationForTeacher(*regTeacher_ok[0]).transact({"from":user})
        tx_res = w3.eth.wait_for_transaction_receipt(tx_hash)
        ev = cont_teacher_0.events.Action.createFilter(fromBlock=blockNumber,toBlock="latest")
        if ev.get_all_entries():
            message = ev.get_all_entries()[0]['args']['text']
        else: message = ""
        status = cont_struct_0.functions.getStatusAddress(user).call({"from":user})
        self.assertEqual(message, "Teacher successfully registered", "Error message")
        self.assertEqual(status, 5, "The teacher did have to register")
        clear_address(user, login)


    # def test_check_reg_wrong_full_empty(self):
    #     user = w3.eth.accounts[9]
    #     login = regTeacher_wrong[0][0]
    #     status = cont_struct_0.functions.getStatusAddress(user).call({"from":user})
    #     if status != 2:
    #         clear_address(user, login)
    #     status = cont_struct_0.functions.getStatusAddress(user).call({"from":user})
    #     self.assertEqual(status, 2, "Error in status Teacher")
    #     blockNumber = w3.eth.block_number
    #     tx_hash = cont_teacher_0.functions.registrationForTeacher(*regTeacher_wrong[0]).transact({"from":user})
    #     tx_res = w3.eth.wait_for_transaction_receipt(tx_hash)
    #     ev = cont_teacher_0.events.Action.createFilter(fromBlock=blockNumber,toBlock="latest")
    #     if ev.get_all_entries():
    #         message = ev.get_all_entries()[0]['args']['text']
    #     else: message = ""
    #     status = cont_struct_0.functions.getStatusAddress(user).call({"from":user})
    #     self.assertEqual(message, "ERROR: empty password or login", "Error message")
    ...
    #         blockNumber = w3.eth.block_number
    #         tx_hash = cont_teacher_0.functions.registrationForTeacher(*data).transact({"from":user})
    #         tx_res = w3.eth.wait_for_transaction_receipt(tx_hash)
    #         ev = cont_teacher_0.events.Action.createFilter(fromBlock=blockNumber,toBlock="latest")
    #         if ev.get_all_entries():
    ...
    #         status = cont_struct_0.functions.getStatusAddress(user).call({"from":user})
    #         self.assertEqual(status, 2, "The teacher didn't have to register")
    #     tx_hash = cont_teacher_0.functions.registrationForTeacher(*regTeacher_wrong[-1]).transact(
    #             {"from":user})
    #     tx_res = w3.eth.wait_for_transaction_receipt(tx_hash)
    #     ev = cont_teacher_0.events.Action.createFilter(fromBlock=blockNumber,toBlock="latest")
    #     if ev.get_all_entries():
    ...
    #         blockNumber = w3.eth.block_number
    #         tx_hash = cont_teacher_0.functions.registrationForTeacher(*data).transact({"from":w3.eth.accounts[8]})
    #         tx_res = w3.eth.wait_for_transaction_receipt(tx_hash)
    #         ev = cont_teacher_0.events.Action.createFilter(fromBlock=blockNumber,toBlock="latest")
    #         if ev.get_all_entries():
    #             # print(ev.get_all_entries()[0]['blockNumber'])
